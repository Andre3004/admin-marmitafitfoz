import type { Cliente, CreateClienteData, FormaPagamento, APIResponse, ClienteAPIData } from '../pages/clientes/types/cliente'
...
export const clientesUseCases = {
    ...
    {
        const response = await fetch(`${HOST}/api/clientes?populate=formaPagamentoPadrao`, {
    ...
    {
        const response = await fetch(`${HOST}/api/clientes/${documentId}?populate=formaPagamentoPadrao`, {
            headers: getAuthHeaders(),
        })
    ...
    async updateCliente(documentId: string, clienteData: Partial<CreateClienteData>): Promise<Cliente>
    {

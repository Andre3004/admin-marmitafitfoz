...
import type { Cliente, CreateClienteData, FormaPagamento } from '../types/cliente'
...
interface ClientesState
{
    ...
    updateCliente: (documentId: string, data: Partial<CreateClienteData>) => Promise<void>
...
export const useClientesStore = create<ClientesState>((set) => ({
    ...
    fetchFormasPagamento: async () =>
    {
        ...
        {
            set({
                error: error instanceof Error ? error.message : 'Erro ao buscar formas de pagamento'
            })
        }
    },
    ...
    updateCliente: async (documentId: string, data: Partial<CreateClienteData>) =>
    {
        set({ isLoading: true, error: null })
        try
        {
            const updatedCliente = await clientesUseCases.updateCliente(documentId, data)
            set(state => ({
                clientes: state.clientes.map(cliente =>
                    cliente.documentId === documentId ? updatedCliente : cliente
                ),
                selectedCliente: state.selectedCliente?.documentId === documentId ? updatedCliente : state.selectedCliente,
                isLoading: false
            }))
        } catch (error)
        {
    ...
    deleteCliente: async (documentId: string) =>
    {
        ...
        try
        {
            ...
            set(state => ({
                clientes: state.clientes.filter(cliente => cliente.documentId !== documentId),
                selectedCliente: state.selectedCliente?.documentId === documentId ? null : state.selectedCliente,
                isLoading: false
            }))
        } catch (error)
        {
